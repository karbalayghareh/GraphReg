                    ...
                    if tf.reduce_sum(tf.gather(tss_idx, idx)) > 0:
                        if (pos[0] > 0 and pos[-1] < 1e15  and gene_tss_list[i] >= pos[0]+2000000 and gene_tss_list[i] < pos[0]+4000000):
                            ...
                            if saliency_method == 'saliency':
                                ...
                                print('cnn: ', scores_cnn.shape, np.min(scores_cnn), np.max(scores_cnn), np.mean(scores_cnn))
...
calculate_loss(cell_line, gene_names_list, gene_tss_list, chr_list, batch_size, saliency_method, write_bw, load_fa, organism, genome)
