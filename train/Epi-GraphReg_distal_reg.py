...
def main():
        ...
        for num, cell_line in enumerate(cell_lines):
            for chr in chr_list:
                if options.generalizable == 0:
                    file_name = data_path+'/data/tfrecords/distal_reg_paper/tfr_epi_'+cell_line+'_'+assay_type+'_FDR_'+fdr+'_'+chr+'.tfr'
                else:
                    file_name = data_path+'/data/tfrecords/distal_reg_paper/tfr_epi_RPGC_'+cell_line+'_'+assay_type+'_FDR_'+fdr+'_'+chr+'.tfr'
    ...
    if re_load:
        ...
        x = Dropout(dropout_rate)(x)
        x = layers.Conv1D(128, 1, activation='relu', padding='same', kernel_regularizer=l2(l2_reg), bias_regularizer=l2(l2_reg))(x)
        x = layers.BatchNormalization()(x)
    ...
    if options.generalizable == 0:
        model_name_gat = data_path+'/models/'+cell_line+'/distal_reg_paper/Epi-GraphReg_'+cell_line+'_'+options.assay_type+'_FDR_'+fdr+'_valid_'+options.valid_chr+'_test_'+options.test_chr+'.h5'
    else:
        model_name_gat = data_path+'/models/'+cell_line+'/distal_reg_paper/Epi-GraphReg_RPGC_'+cell_line+'_'+options.assay_type+'_FDR_'+fdr+'_valid_'+options.valid_chr+'_test_'+options.test_chr+'.h5'
    ...
    for epoch in range(1,n_epochs+1):
        ...
        for num, cell_line in enumerate(cell_lines):
            for chr in train_chr_list:
                if options.generalizable == 0:
                    file_name_train = data_path+'/data/tfrecords/distal_reg_paper/tfr_epi_'+cell_line+'_'+assay_type+'_FDR_'+fdr+'_'+chr+'.tfr'
                else:
                    file_name_train = data_path+'/data/tfrecords/distal_reg_paper/tfr_epi_RPGC_'+cell_line+'_'+assay_type+'_FDR_'+fdr+'_'+chr+'.tfr'
